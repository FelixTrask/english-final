...
function Entry({ entry, originalIndex }) {
  ...
  useGSAP(() => {
    gsap.fromTo(
      `.entry-${originalIndex}`,
      { opacity: 0, y: 20 },
      { opacity: 1, y: 0, delay: originalIndex * 0.1, duration: 0.6, ease: "power2.out" }
    );
  }, []);
  ...
  return (
    <div className={`entry-${originalIndex} mb-8 bg-white rounded-xl shadow-md`}>
      <button
        onClick={toggle}
...
function Chapters() {
  ...
  // Raw entries array (all 11 entries)
  const rawEntries = [
    {
    ...
    },
    {
      ...
      response:
        "Stephen King wrote the book Pet Sematary and although it's not his most popular work, it's still certainly quite chilling. It's pretty obvious that the overarching genre of Pet Sematary is horror, but what type is it exactly? I haven't read too far into the book yet, so I'm still not quite sure. In the book so far, it introduces the main character who is a hospital worker for a nearby college that they just moved near. He has a wife and two kids, and a cat named 'Church' (short for Winston Churchill). He moved to a house on the outskirts of town, right next to a highway and across from a man named Jud, who has lived in the area since he was a child. Jud introduces him to a eerie place deep in the woods called the pet sematary, where the pets that got 'claimed by the road' were buried. This prompts the daughter to have an outburst, yelling that she doesn't want Church to get killed as well. The next day, it's the husband's first day on the job and a man is severly injured by a car accident, eventually dying right next to the husband, with his last words being about the pet sematary and how there's something darker buried under it. I haven't gotten any farther but it's been super fun to read so far. I can't wait to see what happens next in the book!",
      supplemental: "N/A",
    },
    {
      readingAssignment: `"Pet Sematary", Stephen King`,
    ...
    {
      readingAssignment: `Pet Sematary Elevator Pitch`,
      date: "May 27th, 2025",
      context: "N/A",
      skill: "N/A",
      passage: "N/A",
      prompt: "N/A",
      response:
        "<strong>Elevator pitch:</strong> <br /> <br />What if death wasn’t the end - but instead the start of something much, much worse? <br /> <br /> In Pet Sematary by Stephen King, A doctor named Louis moves with his family to a quiet town in Maine, looking for a fresh start. But behind their home lies a path leading deep into the woods… and to an ancient, dark and sinister burial ground. When tragedy hits their family, temptation gets the best of them, and Louis dares to cross the line between life and death. <br /> <br /> Something came back across that line, but it didn’t come back right. <br /> <br /> Twisting, heartbreaking, and utterly horrifying, Pet Sematary is a chilling exploration of the terrible cost of refusing to let go. If you’ve ever thought about what you would do to bring someone back, this book dares you to find out. <br /> <br /> <strong>My opinion on if it should be added to the assigned FLC curriculum (answered in an R&R):</strong> <br /> <br /> I personally think that Pet Sematary should definitely not be recommended for FLC to read. In my opinion, the book was way too gruesome and scary. Instead, maybe another of King's novels could possibly be read. I believe that if it was necessary to read it in FLC, then maybe snippets could be analyzed. However, I would recommend this book to other 9th graders who like to read horror and have maybe read one of Stephen King's novels before. I think that it was beautifully written, and the emotions that were invoked when read were very powerful. I also think that the book could appeal to parents especially, because a lot of the book is centered around losing family and the grief that it comes with. In conclusion, while Pet Sematary is an amazing book, I believe that it wouldn't be fit for FLC to read.",
      supplemental: "N/A",
    }
  ];

  // Attach original index for animations
  const entries = rawEntries.map((entry, idx) => ({ ...entry, originalIndex: idx }));

  // Group by seasons and choice novel
  const fall = entries.slice(0, 5);
  const winter = entries.slice(5, 7);
  const spring = entries.slice(7, 8);
  const choiceNovel = entries.slice(8);
  ...
  return (
    ...
    >
      <div className="max-w-3xl mx-auto">
        ...
        {/* Season: Fall */}
        <h2 className="text-2xl font-semibold text-indigo-700 mb-6">Fall</h2>
        {fall.map(entry => (
          <Entry key={entry.originalIndex} entry={entry} originalIndex={entry.originalIndex} />
        ))}

        {/* Season: Winter */}
        <h2 className="text-2xl font-semibold text-indigo-700 mb-6">Winter</h2>
        {winter.map(entry => (
          <Entry key={entry.originalIndex} entry={entry} originalIndex={entry.originalIndex} />
        ))}

        {/* Season: Spring */}
        <h2 className="text-2xl font-semibold text-indigo-700 mb-6">Spring</h2>
        {spring.map(entry => (
          <Entry key={entry.originalIndex} entry={entry} originalIndex={entry.originalIndex} />
        ))}

        {/* Choice Novel Section */}
        <h2 className="text-2xl font-semibold text-indigo-700 mb-6">Choice Novel</h2>
        {choiceNovel.map(entry => (
          <Entry key={entry.originalIndex} entry={entry} originalIndex={entry.originalIndex} />
        ))}
      </div>
    </div>
